...
install_panel()
{
	...
	if [ $VER = "UbuntuLTS" ]; then
		...
		read -p "${white}Пожалуйста, введите Секретный ключ:${reset}" REKEY
		if [ $? -eq 0 ]; then
			echo "${green}[SUCCESS] "
			tput sgr0
		else
		...
		sudo apt-get install -y pwgen apache2 php5.6 php5.6-curl php5.6-curl php5.6-gd php5.6-mysql php5.6-xml php5.6-ssh2 php5.6-mbstring mariadb-server unzip htop sudo curl screen > /dev/null 2>&1
		...
		log_n "${BLUE}*******************| Скачиваем панель web |*******************"
		cd / > /dev/null 2>&1
		wget https://github.com/Alexeyot/Hostredline/raw/main/web.zip > /dev/null 2>&1
		if [ $? -eq 0 ]; then
			echo "${green}[SUCCESS]"
			tput sgr0
		else
		...
		log_n "${BLUE}*******************| Распаковка панеля web |*******************"
		unzip web.zip -d /var/www/ > /dev/null 2>&1
		rm web.zip > /dev/null 2>&1
		cd > /dev/null 2>&1
		if [ $? -eq 0 ]; then
			echo "${green}[SUCCESS]"
			tput sgr0
		...
		sed -i "s/crontoke12/${CRONTOKE}/g" /var/www/application/config.php
		sed -i "s/KeyCaptcha/${REKEY}/g" /var/www/application/config.php
		if [ $? -eq 0 ]; then
			echo "${green}[SUCCESS]"
			tput sgr0
		else
		...
		mysql hostin < /var/www/host.sql > /dev/null 2>&1
		rm -rf /var/www/host\.sql > /dev/null 2>&1
		if [ $? -eq 0 ]; then
			echo "${green}[SUCCESS]"
			tput sgr0
		else
